...
public class GlobalExceptionHandler {
    ...
    public ResponseEntity<String> handler(Exception e) {
        ...
        return ResponseEntity.status(INTERNAL_SERVER_ERROR)
                .contentType(MediaType.APPLICATION_JSON).body("Ошибка сервера app-order");
    ...
    public ResponseEntity<String> handler(EntityNotFoundException e) {
        ...
        return ResponseEntity.status(SERVICE_UNAVAILABLE)
                .contentType(MediaType.APPLICATION_JSON).body("Ошибка поиска app-order");
    ...
    public ResponseEntity<String> handler(ExternalServiceInteractionException e) {
        log.error("Ошибка: {}", e.getMessage());
        return ResponseEntity.status(INTERNAL_SERVER_ERROR)
                .contentType(MediaType.APPLICATION_JSON).body("Ошибка обращения к сервису app-billing");
